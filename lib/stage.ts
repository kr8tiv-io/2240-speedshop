    ...
    shader.fragmentShader = shader.fragmentShader
      ...
      .replace(
        ...
        /* The uDiss guard is a hard visibility guarantee: at full reveal a car
           ...
           An act shipped invisible for weeks because a stale uniform kept the
           front negative across the whole body while every scene-graph check
           (visible, framed, lit) passed — this makes that entire failure class
           render as, at worst, a wrongly-timed build, never an absent car. */
...
export const cloudVertex = /* glsl */ `
  ...
  void main() {
    ...
    /* CLAMPED, and this clamp is load-bearing. The old size was
       ...
       exactly as long as the camera dolly kept it close. Dark cloud = the
       black rectangle over act II; bright cloud on the white build = the
       white flash. The distance floor comes up to 1.0 (nothing this scene
       frames sits closer than a metre) and the pixel size caps at 42 —
       big enough for every legitimate near-camera mote, far too small to
       read as a slab. Points actually BEHIND the camera are zeroed outright:
       their projection is meaningless and they can only ever be artefact. */
    float dist = -mv.z;
