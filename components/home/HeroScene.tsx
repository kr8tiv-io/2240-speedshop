...
function ActStage({
  ...
  /* ACT I only: the finished car sits on a turntable. Time-based, not
     scroll-based — the car keeps turning while the visitor reads, which is
     what makes the shot feel filmed rather than scrubbed. */
  ...
  const built = useMemo(() => {
    ...
    if (!realBox.isEmpty()) {
      /* De-mount the measurement. setFromObject reads WORLD space, and
         whether the fitted groups are mounted when this memo runs depends on
         act order and refresh timing — measured live, act I's box came out
         2.4× (raw model units) while act II's was correct (fitted metres).
         An inflated box pushes the analytic fit way out and the car reads as
         a speck under the lamp on phones. Express the box in the GLTF root's
         LOCAL space, then apply the KNOWN fit transform by hand: exact for
         rotY ∈ {0, π/2} and uniform scale, and identical no matter when the
         memo runs relative to mounting. */
      scene.updateMatrixWorld(true);
      const inv = new THREE.Matrix4().copy(scene.matrixWorld).invert();
      realBox.applyMatrix4(inv);
      const size = realBox.getSize(new THREE.Vector3());
      const centre = realBox.getCenter(new THREE.Vector3());
      const hx = (size.x / 2) * fit.scale;
      const hy = (size.y / 2) * fit.scale;
      const hz = (size.z / 2) * fit.scale;
      HALF[index].set(fit.rotY ? hz : hx, hy, fit.rotY ? hx : hz);
      CENTRE_Y[index] = (centre.y - fit.localMinY) * fit.scale;
    }
    /* ACT I turns on a showroom table now. The box the rig frames against is
       the honest measured one — the rig rotates its corners by the live
       turntable angle instead (see TURNTABLE in the Rig), because inflating
       the box to the swept cylinder pushed portrait phones so far back the
       car read as a speck under the lamp. */
  ...
  useFrame((state, delta) => {
    ...
    /* The turntable. Speed eases in once the reveal has finished building
       ...
       48-second lap — the cinematic build's plinth speed. Clamped delta so
       a tab-switch cannot teleport the car half a turn. */
    if (index === 0 && spinRef.current) {
      const ease = THREE.MathUtils.clamp((stage.local - 0.28) / 0.14, 0, 1);
      if (on && ease > 0) {
        spinAngle.current += Math.min(delta, 0.05) * 0.13 * ease;
        spinRef.current.rotation.y = built.fit.rotY + spinAngle.current;
        if (deckRef.current) deckRef.current.rotation.y = spinAngle.current;
      }
      TURNTABLE.angle = spinAngle.current;
    }
  ...
  return (
    <group ref={group} visible={false}>
      ...
      {/* THE TURNTABLE (act I only). A showroom table drawn on the floor:
          one static kerb ring, and two radial hairlines that turn with the
          car — the motion cue that sells the rotation without a raised deck
          fighting the contact shadow. Emissive sits well under the bloom
          threshold: it should read as painted metal, not neon. */}
      {index === 0 && (
        <>
          <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.006, 0]} renderOrder={2}>
            <ringGeometry
              args={[
                Math.max(built.fit.half.x, built.fit.half.z) * 1.06,
                Math.max(built.fit.half.x, built.fit.half.z) * 1.12,
                72,
              ]}
            />
          ...
          <group ref={deckRef}>
            {[0, Math.PI / 2].map((a) => (
              <mesh key={a} rotation={[0, a, 0]} position={[0, 0.02, 0]} renderOrder={2}>
                <boxGeometry
                  args={[Math.max(built.fit.half.x, built.fit.half.z) * 2.04, 0.006, 0.028]}
                />
                <meshStandardMaterial
                  color="#2b251c"
                  emissive={TUNGSTEN}
...
const spinHalf = new THREE.Vector3();
...
function cornerEdge(
  ...
  /** Y-rotation of the box (the turntable) — corners are checked where the
      car actually is, not where its rest-pose AABB was. */
  rotY = 0,
) {
  ...
  let worst = 0;
  for (let i = 0; i < 8; i++) {
    const x = i & 1 ? half.x : -half.x;
    const z = i & 4 ? half.z : -half.z;
    probe
      .set(x * c + z * s, i & 2 ? half.y : -half.y, -x * s + z * c)
      .add(centre)
      .project(camera);
...
function Rig({ mobile }: { mobile: boolean }) {
  ...
  useFrame((_, delta) => {
    ...
    if (spin !== 0) {
      const ac = Math.abs(Math.cos(spin));
      const as = Math.abs(Math.sin(spin));
      spinHalf.set(
        HALF[0].x * ac + HALF[0].z * as,
        HALF[0].y,
        HALF[0].x * as + HALF[0].z * ac,
      );
      fitHalf = spinHalf;
    }
    ...
    for (let i = 0; i < 3; i++) {
      const edge = cornerEdge(camera, HALF[stage.act], fitPoint, spin);
