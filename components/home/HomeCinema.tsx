 ...
 *
 * The combined page walks like this: Acts I and II play on the first runway,
 * the page then rolls into the 3D shop walk-through (its own world, passed in
 * as `walkthrough`), and Act III waits at the far end as the finale runway.
 * One fixed canvas serves both runways; between them it is faded out and its
 * frameloop parked, so the walk-through owns the GPU.
 *
 * `shot.film` (0 → 3) is still the entire projector — runway A scrubs 0 → 2,
 * runway C scrubs 2 → 3. Because the reveal curve is zero at BOTH ends of
 * every act, the value parking at exactly 2.0 during the whole walk-through
 * means the stage is empty on every frame the hero canvas isn't wanted.
 *
 *   ACT I    the finished car     Challenger, oxblood candy, on the turntable
 *   ACT II   the bay              1972 coupe, hood up, lamp down in the motor
 *   — the walk-through plays here —
 *   ACT III  the donor            Charger, moonlight through the door
 *
...
/** Beats are absolute on runway A's 200-beat timeline. One act = 100. */
const ACT = [0, 100];
  ...
  useGSAP(
    () => {
      ...
      /* The act HUD: mono readout, room caption and a 1px progress rule,
         ticked from runway A's ScrollTrigger. Runway C carries its own
         static ACT 03 readout — a walk through the shop separates them. */
      ...
      const ROOMS = ["ONE LAMP ON", "HOOD UP · BAY TWO"];
      let lastAct = -1;

      /* THE CANVAS CURTAIN. One fixed canvas serves both runways; between
         them the walk-through world owns the screen. autoAlpha (opacity +
         visibility) so the hidden canvas also stops hit-testing. */
      ...
      const runwayA = wrap.querySelector<HTMLElement>("[data-runway-a]");
      const runwayC = wrap.querySelector<HTMLElement>("[data-runway-c]");
      if (!runwayA || !runwayC) return;

      /* Runway A: 200 beats — Acts I and II. */
      const tl = gsap.timeline({
        ...
        scrollTrigger: {
          ...
          scrub: 1,
          onLeave: () => showCanvas(false),
          onEnterBack: () => showCanvas(true),
          onUpdate: (self) => {
            ...
            const act = Math.min(1, Math.floor(self.progress * 2));
            if (act !== lastAct) {
              lastAct = act;
              if (hudAct) {
      ...
      /* THE PROJECTOR, reel one. film 0 → 2 across runway A. */
      tl.to(shot, { film: 2, duration: 200 }, 0);

      /* Runway C: 100 beats — Act III, after the walk-through. */
      const tlC = gsap.timeline({
        ...
        scrollTrigger: {
          ...
          scrub: 1,
          onEnter: () => showCanvas(true),
          onLeaveBack: () => showCanvas(false),
          /* After the finale the page continues as DOM sections — the fixed
             canvas has to leave with its runway or it sits behind them. */
          onLeave: () => showCanvas(false),
          onEnterBack: () => showCanvas(true),
        },
      });
      tlC.to(shot, { film: 3, duration: 100 }, 0);

      /* Deep links land wherever they land: if the page wakes up between the
         reels (or past both), the curtain starts down — onLeave only fires on
         transitions, not on arrival. */
      const stA = tl.scrollTrigger;
      ...
      /* ── INTERLUDE 02 — the walk-in beat. It plays out the end of runway A
         and hands the page to the shop: the canvas curtain drops right after
         it fades. ─────────────────────────────────────────────────────── */
      tl.fromTo(
        q("[data-interlude='1']"),
        { autoAlpha: 0, y: 34 },
      ...
      tl.to(q("[data-interlude='1']"), { autoAlpha: 0, y: -30, duration: 5 }, ACT[1] + 95);
      ...
      /* ── ACT III — the donor, on its own runway after the walk-through ── */
      tlC.fromTo(
        q("[data-slate='2']"),
        { autoAlpha: 0, y: 18 },
        { autoAlpha: 1, y: 0, duration: 5 },
      ...
      tlC.to(q("[data-slate='2']"), { autoAlpha: 0, duration: 5 }, 40);
      tlC.fromTo(
        q("[data-chapter='3']"),
        { autoAlpha: 0, y: 46 },
        { autoAlpha: 1, y: 0, duration: 7 },
      ...
      tlC.to(shot, { cool: 1, duration: 12 }, 12);
      tlC.to(shot, { lamp: 0.45, duration: 12 }, 40);
      tlC.to(shot, { tail: 1, duration: 12 }, 62);

      /* Display echoes inside the film: the outline layer drifts against its
         bone face across both reels. */
    ...
    <div ref={wrapRef} className="relative">
      ...
      <div data-film data-film-runway data-runway-a className="relative z-[1] h-[850vh]">
        <div className="sticky top-0 h-[100svh] overflow-hidden">
          ...
          {/* INTERLUDE 02 — the walk-in beat: the last thing you read before
              the page hands you the shop itself. */}
          <Interlude
            index={1}
            ...
            body="Gaps, reveals, the way a body line carries from the front fender through the door and lands on the quarter. That work happens in bare metal and primer, months before anything gets sprayed. The next best thing to standing in the room is walking it — keep scrolling."
            stat={{ figure: "One", caption: "person owns your build start to finish. You will have his number." }}
          />
        </div>
      </div>

      {/* THE WALK-THROUGH — the shop itself, its own world, between the reels. */}
      {walkthrough}
        ...
        <div className="sticky top-0 h-[100svh] overflow-hidden">
          ...
          <div className="pointer-events-none absolute inset-x-0 top-[72px] flex justify-end px-5 sm:px-8">
            <div className="text-right">
              <p className="corner-note hidden sm:block">AFTER HOURS · BACK BAY · MOONLIGHT</p>
              <p className="corner-note mt-1.5">
                <span className="text-tungsten">ACT 03</span>
                <span className="text-steel/40"> / 03</span>
              </p>
            </div>
          </div>
