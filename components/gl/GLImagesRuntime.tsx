...
const PLANE_VERT = /* glsl */ `
  ...
  void main() {
    ...
    // The velocity SKEW that used to live here (pos.y shear, the shader twin
    // of the DOM [data-skew] effect) is gone with its DOM sibling: the client
    // read it, correctly, as images "coming in on an angle and fluttering".
...
const PLANE_FRAG = /* glsl */ `
  ...
  void main() {
    ...
    vec3 col = vec3(r, g, b);
    // the night grade (shader twin of .graded)
    col = (col - 0.5) * 1.12 + 0.5;
    col *= vec3(1.02, 0.985, 0.945);
    // flashlight lift near the cursor
    ...
    float light = smoothstep(0.85, 0.0, d) * uHover;
    col *= mix(0.6, 1.04, light);
    col += vec3(1.0, 0.82, 0.6) * light * 0.055;
...
export function GLImagesRuntime({
  active,
  onContextLost,
}: {
  active: boolean;
  ...
  return (
    <div data-gl-images className="pointer-events-none fixed inset-x-0 top-0 z-30 h-[100svh] w-full" aria-hidden="true">
      <Canvas
...
export default GLImagesRuntime;
