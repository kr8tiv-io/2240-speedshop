...
export function CostTable({ heading, intro, rows, footnote, id = "costs" }: Props) {
  return (
    <section aria-labelledby={id} className="mt-20">
      ...
      <p className="mt-6 max-w-3xl border-l-2 border-speed-red bg-panel/70 px-4 py-3 font-mono text-xs leading-relaxed text-tungsten">
      ...
      <div className="plate mt-6 overflow-x-auto">
        <table className="w-full min-w-[36rem] border-collapse text-left">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.item} className="border-b border-rust/15 last:border-0">
                <th
                  scope="row"
      ...
      <p className="mt-2 max-w-3xl text-sm leading-relaxed text-steel">
        ...
        at{" "}
        <a className="font-mono text-tungsten underline decoration-rust underline-offset-4 hover:text-bone" href={`tel:${site.phone}`}>
          {site.phoneDisplay}
        </a>
        .
      </p>
