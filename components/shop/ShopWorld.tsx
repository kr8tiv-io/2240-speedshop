...
import { fadeGlow, useConeGlow, useRadialGlow } from "./materials";
import {
  ARC_BLUE,
  BAY_BLACK,
  CEIL,
...
function CameraRig() {
  ...
  useEffect(() => {
    const read = () => {
      ...
      // the combined page. `runwayProgress` keeps the original rig's
      // three-way iOS scroll read (window.scrollY / documentElement.scrollTop
  ...
  useFrame((state, delta) => {
    ...
    /* The RAIL integrates on (nearly) real wall-clock time, not the clamped
       step. While a bay pays its shader bill a frame can run seconds; with dt
       capped at 0.1 s the eased scroll only advanced ~13% of its gap per
       FRAME, so a stretch of slow frames parked the camera a full station
       short of where the page actually was — mid-glide, occasionally inside a
       prop, which the probe caught as a full-viewport blur wash. Exponential
       damping is unconditionally stable for large dt, so the rail may take
       the real elapsed time; the clamped step still drives everything that
       must stay smooth per-frame (breath, parallax, roll, lens). */
    const railStep = THREE.MathUtils.clamp(delta, 1e-4, 2);
    const elapsed = state.clock.elapsedTime;
...
function FocusRig({ effect }: { effect: RefObject<DofEffect | null> }) {
  ...
  useFrame((state) => {
    ...
    // TRAVEL FOCUS. On an arc the rack focus is the shot: subject sharp, far
    // end of the shop falling away. Mid-GLIDE the look point is the empty
    // floor between two subjects, and holding a shallow lens on empty air
    // renders the whole aisle soft — which reads as a defect the moment the
    // reader rests mid-travel (the combined page lets them rest anywhere).
    // So the iris closes through the travels: bokeh eases toward a deep-focus
    // walk between stations and reopens as the next orbit begins. Mutating
    // `bokehScale` writes one uniform — no rebuild.
    const s = railOf(state.camera).t * SEGMENTS;
    const frac = s - Math.floor(s);
    const travel =
      frac > ORBIT_SPAN && s < SEGMENTS
        ? Math.sin(Math.PI * ((frac - ORBIT_SPAN) / (1 - ORBIT_SPAN)))
        : 0;
    if (effect.current) effect.current.bokehScale = 1.3 * (1 - 0.8 * travel);
  });
...
export function ShopWorld({
  ...
  /* Tell the streaming layer whether anyone can see this canvas — while the
     ...
     manages to be warm before the reader walks in. */
  useEffect(() => {
    setWorldParked(!(awake && warm && active));
    return () => setWorldParked(true);
  }, [awake, warm, active]);
    ...
    >
      <Canvas
        ...
        onCreated={({ gl, scene, camera }) => {
          ...
          /* Off in dev as well as prod ON THIS PAGE — a deviation from the
             original, measured into: with the checks on, every warm slice
             blocks on getShaderInfoLog while D3D translates (5–9 SECONDS per
             slice in the dev profile), the pacing deadlines all fire, and the
             stream crawls so far behind the reader that the bays never seem
