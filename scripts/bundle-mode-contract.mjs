...
assert.match(
  deploy,
  /NODE_OPTIONS[\s\S]*--use-system-ca[\s\S]*node --use-system-ca node_modules\/next\/dist\/bin\/next build --webpack/,
  "The Windows export must keep TLS verification while trusting its system CA store.",
);
assert.match(
  deploy,
  /\.deploy-current-static\.txt[\s\S]*\.deploy-current-shelves\.txt[\s\S]*preserving previous immutable runtime\/model generation[\s\S]*Copy-Item -LiteralPath \$item\.FullName -Destination \$Repo -Recurse -Force[\s\S]*restoring previous immutable runtime\/model generation/,
