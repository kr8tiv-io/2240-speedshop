    ...
    <article className="mx-auto max-w-6xl px-5 py-14 sm:py-20">
      ...
      <header className="mt-8 grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
        ...
        <figure className="lg:sticky lg:top-24">
          <div className={`relative w-full overflow-hidden border border-rust/35 ${hero.aspect}`}>
            <div
              className="absolute inset-0"
              style={hero.zoom ? { transform: `scale(${hero.zoom})` } : undefined}
            >
              <Image
                src={hero.src}
                alt={hero.alt}
          ...
          <ul className="mt-8 grid items-start gap-8 sm:grid-cols-2">
            {gallery.map((img) => (
              <li key={img.src}>
                <figure>
                  <div
                    className={`relative w-full overflow-hidden border border-rust/35 ${img.aspect}`}
                  >
                    <div
                      className="absolute inset-0"
                      style={img.zoom ? { transform: `scale(${img.zoom})` } : undefined}
                    >
                      <Image
                        src={img.src}
                        alt={img.alt}
      ...
      <section aria-labelledby="more-builds" className="mt-20">
        ...
        <ul className="mt-8 grid items-start gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {rest.map((b) => (
            <li key={b.slug}>
              <Link href={`/builds/${b.slug}`} className="plate group block h-full overflow-hidden">
                <div className={`relative w-full overflow-hidden ${b.images[0].aspect}`}>
                  <div
                    className="absolute inset-0"
                    style={
                      b.images[0].zoom ? { transform: `scale(${b.images[0].zoom})` } : undefined
                    }
                  >
                    <Image
                      src={b.images[0].src}
